...
def add_factories(player: Player, types: Iterable[str]) -> None:
    for factory_type in types:
        monthly = (
            1_500.0 if factory_type in {"auto", "builds_auto"} else 1_000.0
        )
...
def test_buy_bids_sorted_by_price_then_priority_and_respect_limits() -> None:
    ...
    session = GameSession(players=players, settings=settings)
    session.set_market()
...
def test_start_production_respects_costs_and_upgrade_factories() -> None:
    ...
    session = GameSession(players=[player], settings=make_settings())
...
def test_build_basic_factory_tracks_second_payment_and_limit() -> None:
    ...
    session = GameSession(players=[player], settings=settings)
...
def test_game_finishes_by_month_limit_and_selects_highest_capital() -> None:
    ...
    session = GameSession(players=[p1, p2], settings=settings)
...
def test_game_finishes_when_only_one_player_survives() -> None:
    ...
    bankrupt.factories.append(
        Factory(factory_type="basic", monthly_expenses=1_000.0)
    )

    session = GameSession(players=[survivor, bankrupt], settings=settings)
